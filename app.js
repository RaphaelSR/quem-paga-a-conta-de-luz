...
async function boot(){
 ...
 function renderMap(){
  ...
  const shown=plants.filter(p=>(!uf||p.uf===uf)&&(small||p.type==='UHE')&&norm(p.name+' '+p.city+' '+p.uf).includes(q));
 ...
 ['#state','#small-plants'].forEach(id=>$(id).addEventListener('change',renderMap));$('#plant-search').addEventListener('input',renderMap);$('#reset-map').addEventListener('click',()=>{$('#state').value='';$('#plant-search').value='';$('#small-plants').checked=false;selectedPlant=null;renderMap();});
